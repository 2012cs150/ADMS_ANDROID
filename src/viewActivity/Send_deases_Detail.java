...
/*
 * This class handle all activity belong to
 * Diseases send class. 
 * 
 */
...
public class Send_deases_Detail extends Activity {
	...
	//create necessary flags to control events.
	 ...
	 * Initialize components ,make object and binding event to  
	 * that components.
	 * 
	 */
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		...
		/*
		 * add listener to region text filed to check the validation.
		 */
		...
		sendDeseaseDetails.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				String pn = plantName.getText().toString();
				long dt = diseaseSpinner.getSelectedItemId();
				long wc = weatherSpinner.getSelectedItemId();
				long sc = soilConditionSpinner.getSelectedItemId();
				long di = districSpinner.getSelectedItemId();
				String rg = region.getText().toString();
				//Log.e("enable", "working//////"+dt);
				if(!(pn.equals("")) && (dt!=0) && (wc!=0) &&(sc!=0) &&(di!=0) && !(rg.equals(""))&& valid_region){
					new sendDetail().execute();
				}else{
					Toast.makeText(Send_deases_Detail.this, "please, check you fill details correctly..!",Toast.LENGTH_LONG).show();
				}
	...
	//==================valid field================
	public void Is_Valid_Region(EditText edt) throws NumberFormatException {
	...
	/*
	 * 
	 * this is a background process class. this class handle 
	 * all necessary details relative to send disease details.
	 *  
	 * 
	 */
